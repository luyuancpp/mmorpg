...
def scanservice():
    for (dirpath, dirnames, filenames) in os.walk(logicprotodir):
        for filename in filenames:  
            if filename.find('client_player') >= 0 or filename.find('server_player') >= 0 or filename.find('_rg') >= 0:
                continue  
...
gencppfile('./md5/controller_server_service.cpp', buildpublic.controller_file_prefix)
gencppfile('./md5/gs_server_service.cpp', buildpublic.gs_file_prefix)
md5copy('./md5/controller_server_service.cpp', '../../../../controller_server/src/service/logic/controller_server_service.cpp')
