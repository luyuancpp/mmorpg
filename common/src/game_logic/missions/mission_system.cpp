...
bool MissionSystem::IsConditionCompleted(uint32_t condition_id, uint32_t progress_value)
{
...
uint32_t MissionSystem::GetReward(entt::entity player, uint32_t missin_id)
{
	auto try_complete_comp = registry.try_get<MissionsComp>(player);
	if (nullptr == try_complete_comp)
	{
		return kRetMissionPlayerMissionCompNotFound;//协议可以记录哪个消息调用
	}
	auto& complete_ids_ = try_complete_comp->complete_ids_;
...
uint32_t MissionSystem::Accept(entt::entity player, const AcceptMissionP& param)
{
	auto try_complete_comp = registry.try_get<MissionsComp>(player);
	if (nullptr == try_complete_comp)
	{
	...
	auto& missions_ = try_complete_comp->missions_;
	auto& complete_ids_ = try_complete_comp->complete_ids_;
	auto& type_filter_ = try_complete_comp->type_filter_;
	auto& event_missions_classify_ = try_complete_comp->event_missions_classify_;
	...
	auto mission_config_ = registry.get<IMissionConfig*>(player);
	...
	bool check_type_filter = mission_config_->HasMainSubTypeCheck() && mission_sub_type > 0 && registry.any_of<CheckSubType>(player);
...
uint32_t MissionSystem::AcceptCheck(entt::entity player, const AcceptMissionP& param)
{
	return kRetOK;
}

uint32_t MissionSystem::Abandon(entt::entity player, uint32_t mission_id)
{
	auto try_complete_comp = registry.try_get<MissionsComp>(player);
	if (nullptr == try_complete_comp)
	{
	...
	auto begin_times = registry.try_get<MissionBeginTime>(player);//全服任务没有
	if (nullptr != begin_times)
	{
		begin_times->mutable_mission_begin_time()->erase(mission_id);
	}
	DelMissionClassify(player, mission_id);
	return kRetOK;
}

void MissionSystem::CompleteAllMission(entt::entity player)
{
	auto try_complete_comp = registry.try_get<MissionsComp>(player);
	if (nullptr == try_complete_comp)
	{
...
void MissionSystem::receive(entt::entity player, const ConditionEvent& c)
{
	auto try_complete_comp = registry.try_get<MissionsComp>(player);
	if (nullptr == try_complete_comp)
	{
	...
	for (auto lit : classify_missions)
	{
		...
		if (!UpdateMissionByCompareCondition(player, c, mission))
		{
			continue;
		}
	...
	OnMissionComplete(player, c, temp_complete);
}

void MissionSystem::DelMissionClassify(entt::entity player, uint32_t mission_id)
{
	auto try_complete_comp = registry.try_get<MissionsComp>(player);
	if (nullptr == try_complete_comp)
	{
...
bool MissionSystem::UpdateMissionByCompareCondition(entt::entity player, const ConditionEvent& ev, Mission& mission)
{
...
void MissionSystem::OnMissionComplete(entt::entity player, const ConditionEvent& c, const TempCompleteList& temp_complete)
{
	...
	auto& missions_ = try_complete_comp->missions_;
	auto& complete_ids_ = try_complete_comp->complete_ids_;
	bool reward = registry.any_of<MissionReward>(player);
	for (auto& mission_id : temp_complete)
	{
		...
		auto next_time_accpet = registry.try_get<NextTimeAcceptMission>(player);
