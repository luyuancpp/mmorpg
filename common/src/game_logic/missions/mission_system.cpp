...
uint32_t MissionSystem::Accept(const AcceptMissionEvent& accept_event)
{
	...
	CheckCondition(!try_mission_comp->mission_config()->HasKey(accept_event.mission_id()), kRetTableId)

	auto mission_sub_type = try_mission_comp->mission_config()->mission_sub_type(accept_event.mission_id());
	auto mission_type = try_mission_comp->mission_config()->mission_type(accept_event.mission_id());
	if (try_mission_comp->mission_type_not_repeated_)
	{
		const UInt32PairSet::value_type mission_and_mission_subtype_pair(mission_type, mission_sub_type);
		CheckCondition(try_mission_comp->type_filter_.find(mission_and_mission_subtype_pair) != try_mission_comp->type_filter_.end(), kRetMissionTypeRepeated)
	}
	MissionPbComp mission_pb;
	mission_pb.set_id(accept_event.mission_id());
	for (const auto& condition_ids = try_mission_comp->mission_config_->condition_id(accept_event.mission_id());
		const auto condition_id : condition_ids)
	{
		...
		try_mission_comp->event_missions_classify_[condition_row->condition_type()].emplace(accept_event.mission_id());
	}
	try_mission_comp->missions_comp_.mutable_missions()->insert({ accept_event.mission_id(), std::move(mission_pb) });
	if (try_mission_comp->mission_type_not_repeated_)
	{
		const UInt32PairSet::value_type mission_and_mission_subtype_pair(mission_type, mission_sub_type);
		try_mission_comp->type_filter_.emplace(mission_and_mission_subtype_pair);
	}
...
uint32_t MissionSystem::Abandon(const AbandonParam& param)
{
	...
	try_mission_comp->missions_comp_.mutable_missions()->erase(param.mission_id_);
	try_mission_comp->missions_comp_.mutable_complete_missions()->erase(param.mission_id_);
	try_mission_comp->missions_comp_.mutable_mission_begin_time()->erase(param.mission_id_);
...
void MissionSystem::CompleteAllMission(entt::entity player, uint32_t op)
{
	...
	auto& missions_comp_ = try_mission_comp->missions_comp_;
...
void MissionSystem::Receive(const MissionConditionEvent& condition_event)
{
	...
	const auto classify_mission_it = try_mission_comp->event_missions_classify_.find(condition_event.type());
	if (classify_mission_it == try_mission_comp->event_missions_classify_.end())
	{
		return;
	}
	...
	{
		auto mit = try_mission_comp->missions_comp_.mutable_missions()->find(lit);
		if (mit == try_mission_comp->missions_comp_.mutable_missions()->end())
		{
			continue;
		}
		...
		const auto& conditions = try_mission_comp->mission_config_->condition_id(mission.id());
		...
		try_mission_comp->missions_comp_.mutable_missions()->erase(mit);
	}
...
void MissionSystem::DeleteMissionClassify(entt::entity player, uint32_t mission_id)
{
	...
	const auto& config_conditions = try_mission_comp->mission_config_->condition_id(mission_id);
	for (int32_t i = 0; i < config_conditions.size(); ++i)
	{
		...
		try_mission_comp->event_missions_classify_[condition_row->condition_type()].erase(mission_id);
	}
	if (auto mission_sub_type = try_mission_comp->mission_config_->mission_sub_type(mission_id);
		mission_sub_type > 0 &&
		try_mission_comp->mission_type_not_repeated_)
	{
		const UInt32PairSet::value_type mission_and_mission_subtype_pair(try_mission_comp->mission_config_->mission_type(mission_id), mission_sub_type);
		try_mission_comp->type_filter_.erase(mission_and_mission_subtype_pair);
	}
}
...
bool MissionSystem::UpdateMission(const MissionConditionEvent& condition_event,	MissionPbComp& mission)
{
	...
	const auto& mission_conditions = try_mission_comp->mission_config_->condition_id(mission.id());
...
void MissionSystem::OnMissionComplete(entt::entity player, const UInt32Set& completed_missions_this_time)
{
	...
	for (const auto& mission_id : completed_missions_this_time)
	{
		try_mission_comp->missions_comp_.mutable_complete_missions()->insert({ mission_id, true });
		...
		else if (nullptr != try_mission_reward && try_mission_comp->mission_config_->reward_id(mission_id) > 0)
		{
			//手动领奖
