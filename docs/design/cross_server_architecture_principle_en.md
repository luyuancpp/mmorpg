# MMO Cross-Server Architecture (Global Design Document)

## Game Model
- MMO (World of Warcraft style), ~1000 realms
...
> No matter which server a player is on, the processing logic is the same as on their home server — no need to check "is this cross-server?".
...
## Chosen Architecture: Plan A — Independent Data Service
...
- Horizontally scalable: shard by realm range (instance 1 → realms 1~200, instance 2 → realms 201~400, etc.)
...
- **Zone**: game-logical realm, e.g. Zone 1~10000
- **Region**: grouping of zones, e.g. Region A = Zone 1~100
- Redis allocated **per Region**, NOT per Zone: 10000 zones / 100 per region = 100 Redis Clusters
- **Lock Region**: if a Region is locked, players can only visit zones within their home Region; if unlocked, free to visit any zone 1~10000
...
- **DO NOT encode zone_id in player_id** — this breaks on server merge
