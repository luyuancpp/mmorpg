...
TEST(TeamManger, CreateFullDismiss)
{
	...
	{	
		m.set_player_id(player_id++);
		GameGuid nTeamId = kEmptyGameGuid;
		EXPECT_EQ(ReturnValue(re::RET_OK), team_list.CreateTeam(p));
	...
	m.set_player_id(player_id++);
	EXPECT_EQ(re::RET_TEAM_TEAM_LIST_MAX, team_list.CreateTeam({ m.player_id(), Members{{m.player_id(), m}}, "" }));

	EXPECT_EQ(TeamList::kMaxTeamSize, team_list.team_size());

	player_id = 1;
	for (PlayerIdsV::iterator it = teamidlist.begin(); it != teamidlist.end(); ++it)
	{
		EXPECT_EQ(re::RET_OK, team_list.DissMissTeam(*it, player_id++));
	}
...
TEST(TeamManger, TeamSizeTest)
{
	...
	TeamMember m;
	m.set_player_id(100);
	EXPECT_EQ(re::RET_OK, team_list.CreateTeam({ m.player_id(), Members{{m.player_id(), m}}, "" }));
	EXPECT_TRUE(team_list.PlayerInTeam(team_list.last_team_id(), m.player_id()));
	EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.JoinTeam(team_list.last_team_id(), m));
	...
	for (std::size_t i = 1; i < kMaxMemberSize; ++i)
	{
		...
		EXPECT_EQ(re::RET_OK, team_list.JoinTeam(team_list.last_team_id(), m));
		EXPECT_EQ(1 + i, team_list.member_size(team_list.last_team_id()));
	}
	m.set_player_id(m.player_id() + 1);
	EXPECT_EQ(re::RET_TEAM_MEMBERS_FULL, team_list.JoinTeam(team_list.last_team_id(), m));
...
TEST(TeamManger, LeaveTeam)
{
	...
	EXPECT_EQ(re::RET_TEAM_HAS_NOT_TEAM_ID, team_list.JoinTeam(team_list.last_team_id(), m));
	EXPECT_EQ(0, team_list.member_size(team_list.last_team_id()));
...
TEST(TeamManger, KickTeaamMember)
{
	...
	EXPECT_EQ(re::RET_TEAM_KICK_SELF, team_list.KickMember(team_list.last_team_id(), m.player_id(), m.player_id()));
	EXPECT_EQ(re::RET_TEAM_KICK_NOT_LEADER, team_list.KickMember(team_list.last_team_id(),99, 99));

	m.set_player_id(m.player_id() + 1);
	EXPECT_EQ(re::RET_OK, team_list.JoinTeam(team_list.last_team_id(), m));
	EXPECT_EQ(re::RET_TEAM_KICK_SELF, team_list.KickMember(team_list.last_team_id(),101, m.player_id()));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	EXPECT_EQ(re::RET_TEAM_KICK_NOT_LEADER, team_list.KickMember(team_list.last_team_id(),101, 101));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	EXPECT_EQ(re::RET_TEAM_KICK_SELF, team_list.KickMember(team_list.last_team_id(), m.player_id(), m.player_id()));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	EXPECT_EQ(re::RET_TEAM_MEMBER_NOT_IN_TEAM, team_list.KickMember(team_list.last_team_id(),100, 88));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	EXPECT_EQ(re::RET_OK, team_list.KickMember(team_list.last_team_id(),100, 101));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
}
...
TEST(TeamManger, AppointLaderAndLeaveTeam1)
{
	...
	EXPECT_EQ(re::RET_TEAM_APPOINT_SELF, team_list.AppointLeader(team_list.last_team_id(), 101, 101));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	EXPECT_EQ(re::RET_TEAM_APPOINT_SELF, team_list.AppointLeader(team_list.last_team_id(), 101, 100));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	EXPECT_EQ(re::RET_TEAM_APPOINT_SELF, team_list.AppointLeader(team_list.last_team_id(), 100, 100));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));

	EXPECT_EQ(re::RET_OK, team_list.AppointLeader(team_list.last_team_id(), 100, 101));
	EXPECT_EQ(101, team_list.leader_id_by_teamid(team_list.last_team_id()));

	team_list.LeaveTeam(101);
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));

	EXPECT_EQ(re::RET_OK, team_list.AppointLeader(team_list.last_team_id(), 100, 102));
	EXPECT_EQ(m.player_id(), team_list.leader_id_by_teamid(team_list.last_team_id()));
	...
	EXPECT_EQ(re::RET_OK, team_list.AppointLeader(team_list.last_team_id(), 100, 103));
	...
	EXPECT_EQ(re::RET_OK, team_list.AppointLeader(team_list.last_team_id(), 100, 104));
...
TEST(TeamManger, DismissTeam)
{
	...
	EXPECT_EQ(re::RET_TEAM_HAS_NOT_TEAM_ID, team_list.DissMissTeam(0, 104));
	EXPECT_EQ(re::RET_TEAM_DISMISS_NOT_LEADER, team_list.DissMissTeam(team_list.last_team_id(), 104));
	EXPECT_EQ(re::RET_OK, team_list.DissMissTeam(team_list.last_team_id(), 100));
	EXPECT_FALSE(team_list.FindTeamId(team_list.last_team_id()));
}
...
TEST(TeamManger, ApplyFull)
{
	...
	{
		a.set_player_id(i);
		EXPECT_EQ(re::RET_OK, team_list.Apply(team_list.last_team_id(),  a));
		if (i < 9)
		{
...
TEST(TeamManger, AgreeApplicant1)
{
	...
	for (GameGuid i = 1; i < nMax; ++i)
	{
		...
		if (i < kMaxMemberSize)
		{
			EXPECT_EQ(re::RET_OK, team_list.AgreeApplicant(team_list.last_team_id(), i));
			EXPECT_FALSE(team_list.HasApplicant(team_list.last_team_id(), i));
		}
		else
		{
			EXPECT_EQ(re::RET_TEAM_MEMBERS_FULL, team_list.AgreeApplicant(team_list.last_team_id(), i));
			EXPECT_TRUE(team_list.HasApplicant(team_list.last_team_id(), i));
		}
	}
    a.set_player_id(6666);
    EXPECT_EQ(re::RET_TEAM_MEMBERS_FULL, team_list.Apply(team_list.last_team_id(), a));

    EXPECT_EQ(re::RET_OK, team_list.LeaveTeam(2));


    m.set_player_id(2);
    EXPECT_EQ(re::RET_OK, team_list.Apply(team_list.last_team_id(), m));
    EXPECT_EQ(re::RET_OK, team_list.CreateTeam({ m.player_id(), Members{{m.player_id(), m}}, "" }));
    EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.AgreeApplicant(team_list.last_team_id(), 2));
...
TEST(TeamManger, RemoveApplicant)
{
	...
	for (GameGuid i = 0; i < nMax; ++i)
	{
		...
		if (i % 2 == 0)
		{
			EXPECT_EQ(re::RET_OK, team_list.RemoveApplicant(team_list.last_team_id(), a.player_id()));
		}
...
TEST(TeamManger, AgreeApplicant)
{
	...
	for (int32_t i = 0; i < nMax; ++i)
	{
		...
		if (i > nMax - 10)
		{
			EXPECT_EQ(re::RET_OK, team_list.AgreeApplicant(team_list.last_team_id(), a.player_id()));
...
TEST(TeamManger, PlayerTeamId)
{
	...
	EXPECT_EQ(re::RET_OK, team_list.RemoveApplicant(team_list.last_team_id(), m.player_id()));
	EXPECT_FALSE(team_list.PlayerInTeam(m.player_id()));
	EXPECT_EQ(kEmptyGameGuid, team_list.GetTeamId(m.player_id()));


	EXPECT_EQ(re::RET_OK, team_list.Apply(team_list.last_team_id(),  m));
	EXPECT_EQ(re::RET_OK, team_list.AgreeApplicant(team_list.last_team_id(), m.player_id()));
	EXPECT_TRUE(team_list.PlayerInTeam(m.player_id()));
	EXPECT_EQ(team_list.last_team_id(), team_list.GetTeamId(m.player_id()));
	...
	m.set_player_id(1);
	EXPECT_EQ(re::RET_OK, team_list.LeaveTeam(m.player_id()));
	EXPECT_FALSE(team_list.PlayerInTeam(m.player_id()));
	EXPECT_EQ(kEmptyGameGuid, team_list.GetTeamId(m.player_id()));
	...
	m.set_player_id(3);
	EXPECT_EQ(re::RET_OK, team_list.KickMember(team_list.last_team_id(), 2, m.player_id()));
	EXPECT_FALSE(team_list.PlayerInTeam(m.player_id()));
	EXPECT_EQ(kEmptyGameGuid, team_list.GetTeamId(m.player_id()));
	

	EXPECT_EQ(re::RET_OK, team_list.DissMissTeam(team_list.last_team_id(), 2));
	for (std::size_t i = 4; i < 11; ++i)
	{
		EXPECT_FALSE(team_list.PlayerInTeam(m.player_id()));
		EXPECT_EQ(kEmptyGameGuid, team_list.GetTeamId(m.player_id()));
...
TEST(TeamManger, PlayerInTeam)
{
	...
	m.set_player_id(1);
	EXPECT_EQ(re::RET_OK, team_list.CreateTeam({ m.player_id(), Members{{m.player_id(), m}}, "" }));
	EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.CreateTeam({ m.player_id(), Members{{m.player_id(), m}}, "" }));
	EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.JoinTeam(team_list.last_team_id(), m));

	m.set_player_id(2);
	EXPECT_EQ(re::RET_OK, team_list.CreateTeam({ m.player_id(), Members{{m.player_id(), m}}, "" }));
	EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.JoinTeam(team_list.last_team_id(), m));
	EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.Apply(team_list.last_team_id(), m));
	EXPECT_EQ(re::RET_TEAM_MEMBER_IN_TEAM, team_list.AgreeApplicant(team_list.last_team_id(), m.player_id()));


	EXPECT_EQ(re::RET_TEAM_PLAEYR_ID, team_list.LeaveTeam(m.player_id()));
	EXPECT_EQ(re::RET_OK, team_list.LeaveTeam(m.player_id()));
	EXPECT_EQ(re::RET_OK, team_list.JoinTeam(team_list.last_team_id(), m));
...
TEST(TeamManger, AppointLeaderNotInTeam)
{
    ...
    {
        EXPECT_EQ(re::RET_TEAM_HAS_NOT_TEAM_ID, team_list.AppointLeader(team_list.last_team_id(), leaderroleid, i));
    }
}
