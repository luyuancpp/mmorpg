...
void Log2Console(const char* msg, int len)
{
    ...
    if (std::string{ msg, 32, 5 }.compare("TRACE") == 0)
    {
        console_msg = LOG_LIGHT_GRAY + console_msg;
    }
    else if (std::string{ msg, 32, 5 }.compare("DEBUG") == 0)
    {
        console_msg = LOG_WHITE + console_msg;
    }
    if (std::string{ msg, 32, 4 }.compare("INFO") == 0)
    {
        console_msg = LOG_GREEN + console_msg;
    }
    else if (std::string{ msg, 32, 4 }.compare("WARN") == 0)
    {
        console_msg = LOG_YELLOW + console_msg;
    }
    else if (std::string{ msg, 32, 5 }.compare("ERROR") == 0)
    {
