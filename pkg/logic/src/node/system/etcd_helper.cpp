...
void EtcdHelper::PutServiceNodeInfo(const NodeInfo& nodeInfo, const std::string& key) {
    ...
    SendetcdserverpbKVPut(tls.globalNodeRegistry, GetGlobalGrpcNodeEntity(), request);
...
void EtcdHelper::RangeQuery(const std::string& prefix) {
	...
	SendetcdserverpbKVRange(tls.globalNodeRegistry, GetGlobalGrpcNodeEntity(), request);
}
...
void EtcdHelper::StartWatchingPrefix(const std::string& prefix) {
	...
	SendetcdserverpbWatchWatch(tls.globalNodeRegistry, GetGlobalGrpcNodeEntity(), request);
}
...
void EtcdHelper::GrantLease(uint32_t ttlSeconds) {
	...
	SendetcdserverpbLeaseLeaseGrant(tls.globalNodeRegistry, GetGlobalGrpcNodeEntity(), leaseReq);
}
...
void EtcdHelper::PutIfAbsent(const std::string& key, const std::string& newValue, int64_t currentVersion, int64_t lease) {
	...
	SendetcdserverpbKVTxn(tls.globalNodeRegistry, GetGlobalGrpcNodeEntity(), txn);
}
...
void EtcdHelper::RevokeLeaseAndCleanup(int64_t leaseId)
{
	...
	SendetcdserverpbLeaseLeaseRevoke(tls.globalNodeRegistry, GetGlobalGrpcNodeEntity(), request);
}