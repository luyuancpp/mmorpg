...
Run:
    python client/image/wire_qdao_v3_assets.py
"""
...
REPO: pathlib.Path = pathlib.Path(__file__).resolve().parents[2]
IMAGE_DIR: pathlib.Path = REPO / "client" / "image"
...
def collect_ui_entries(next_id: list[int]) -> list[str]:
    ...
    print(f"[ui] wrote {len(entries)} entries to {target_dir.relative_to(REPO)}")
    return entries
...
def collect_weapon_entries(next_id: list[int]) -> list[str]:
    ...
    for root in (WEAPONS_DIR_A, WEAPONS_DIR_B):
        if not root.exists():
            print(f"  [skip] missing weapon dir: {root}", file=sys.stderr)
            continue
        sources.extend(sorted(p for p in root.glob("*.png")
...
def rewrite_package_xml(new_entries: typing.Iterable[str]) -> None:
    ...
    print(f"[package.xml] appended {len(entries)} entries (backup -> {backup.name})")
