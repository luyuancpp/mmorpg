// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.8
// 	protoc        v5.29.0
// source: proto/logic/event/player_migration_event.proto

package game

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type PlayerMigrationPbEvent struct {
	state                protoimpl.MessageState      `protogen:"open.v1"`
	PlayerId             uint64                      `protobuf:"varint,1,opt,name=player_id,json=playerId,proto3" json:"player_id,omitempty"`
	SourceSceneId        uint64                      `protobuf:"varint,2,opt,name=source_scene_id,json=sourceSceneId,proto3" json:"source_scene_id,omitempty"`
	TargetSceneId        uint64                      `protobuf:"varint,3,opt,name=target_scene_id,json=targetSceneId,proto3" json:"target_scene_id,omitempty"`
	SerializedPlayerData []byte                      `protobuf:"bytes,4,opt,name=serialized_player_data,json=serializedPlayerData,proto3" json:"serialized_player_data,omitempty"`
	FromZone             uint32                      `protobuf:"varint,5,opt,name=from_zone,json=fromZone,proto3" json:"from_zone,omitempty"`
	ToZone               uint32                      `protobuf:"varint,6,opt,name=to_zone,json=toZone,proto3" json:"to_zone,omitempty"`
	Timestamp            int64                       `protobuf:"varint,7,opt,name=timestamp,proto3" json:"timestamp,omitempty"` // 事件生成时间
	CentreNodeId         uint32                      `protobuf:"varint,8,opt,name=centre_node_id,json=centreNodeId,proto3" json:"centre_node_id,omitempty"`
	SceneInfo            *ChangeSceneInfoPBComponent `protobuf:"bytes,9,opt,name=scene_info,json=sceneInfo,proto3" json:"scene_info,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *PlayerMigrationPbEvent) Reset() {
	*x = PlayerMigrationPbEvent{}
	mi := &file_proto_logic_event_player_migration_event_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PlayerMigrationPbEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PlayerMigrationPbEvent) ProtoMessage() {}

func (x *PlayerMigrationPbEvent) ProtoReflect() protoreflect.Message {
	mi := &file_proto_logic_event_player_migration_event_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PlayerMigrationPbEvent.ProtoReflect.Descriptor instead.
func (*PlayerMigrationPbEvent) Descriptor() ([]byte, []int) {
	return file_proto_logic_event_player_migration_event_proto_rawDescGZIP(), []int{0}
}

func (x *PlayerMigrationPbEvent) GetPlayerId() uint64 {
	if x != nil {
		return x.PlayerId
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetSourceSceneId() uint64 {
	if x != nil {
		return x.SourceSceneId
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetTargetSceneId() uint64 {
	if x != nil {
		return x.TargetSceneId
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetSerializedPlayerData() []byte {
	if x != nil {
		return x.SerializedPlayerData
	}
	return nil
}

func (x *PlayerMigrationPbEvent) GetFromZone() uint32 {
	if x != nil {
		return x.FromZone
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetToZone() uint32 {
	if x != nil {
		return x.ToZone
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetTimestamp() int64 {
	if x != nil {
		return x.Timestamp
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetCentreNodeId() uint32 {
	if x != nil {
		return x.CentreNodeId
	}
	return 0
}

func (x *PlayerMigrationPbEvent) GetSceneInfo() *ChangeSceneInfoPBComponent {
	if x != nil {
		return x.SceneInfo
	}
	return nil
}

var File_proto_logic_event_player_migration_event_proto protoreflect.FileDescriptor

const file_proto_logic_event_player_migration_event_proto_rawDesc = "" +
	"\n" +
	".proto/logic/event/player_migration_event.proto\x1a&proto/logic/component/scene_comp.proto\"\xf1\x02\n" +
	"\x16PlayerMigrationPbEvent\x12\x1b\n" +
	"\tplayer_id\x18\x01 \x01(\x04R\bplayerId\x12&\n" +
	"\x0fsource_scene_id\x18\x02 \x01(\x04R\rsourceSceneId\x12&\n" +
	"\x0ftarget_scene_id\x18\x03 \x01(\x04R\rtargetSceneId\x124\n" +
	"\x16serialized_player_data\x18\x04 \x01(\fR\x14serializedPlayerData\x12\x1b\n" +
	"\tfrom_zone\x18\x05 \x01(\rR\bfromZone\x12\x17\n" +
	"\ato_zone\x18\x06 \x01(\rR\x06toZone\x12\x1c\n" +
	"\ttimestamp\x18\a \x01(\x03R\ttimestamp\x12$\n" +
	"\x0ecentre_node_id\x18\b \x01(\rR\fcentreNodeId\x12:\n" +
	"\n" +
	"scene_info\x18\t \x01(\v2\x1b.ChangeSceneInfoPBComponentR\tsceneInfoB\tZ\apb/gameb\x06proto3"

var (
	file_proto_logic_event_player_migration_event_proto_rawDescOnce sync.Once
	file_proto_logic_event_player_migration_event_proto_rawDescData []byte
)

func file_proto_logic_event_player_migration_event_proto_rawDescGZIP() []byte {
	file_proto_logic_event_player_migration_event_proto_rawDescOnce.Do(func() {
		file_proto_logic_event_player_migration_event_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_logic_event_player_migration_event_proto_rawDesc), len(file_proto_logic_event_player_migration_event_proto_rawDesc)))
	})
	return file_proto_logic_event_player_migration_event_proto_rawDescData
}

var file_proto_logic_event_player_migration_event_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_proto_logic_event_player_migration_event_proto_goTypes = []any{
	(*PlayerMigrationPbEvent)(nil),     // 0: PlayerMigrationPbEvent
	(*ChangeSceneInfoPBComponent)(nil), // 1: ChangeSceneInfoPBComponent
}
var file_proto_logic_event_player_migration_event_proto_depIdxs = []int32{
	1, // 0: PlayerMigrationPbEvent.scene_info:type_name -> ChangeSceneInfoPBComponent
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_proto_logic_event_player_migration_event_proto_init() }
func file_proto_logic_event_player_migration_event_proto_init() {
	if File_proto_logic_event_player_migration_event_proto != nil {
		return
	}
	file_proto_logic_component_scene_comp_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_logic_event_player_migration_event_proto_rawDesc), len(file_proto_logic_event_player_migration_event_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_proto_logic_event_player_migration_event_proto_goTypes,
		DependencyIndexes: file_proto_logic_event_player_migration_event_proto_depIdxs,
		MessageInfos:      file_proto_logic_event_player_migration_event_proto_msgTypes,
	}.Build()
	File_proto_logic_event_player_migration_event_proto = out.File
	file_proto_logic_event_player_migration_event_proto_goTypes = nil
	file_proto_logic_event_player_migration_event_proto_depIdxs = nil
}
