...
using CooldownTableTempPtr = WarnOnSavePtr<const CooldownTable>;

class CooldownTableManager {
public:
    using KeyValueDataType = std::unordered_map<uint32_t, const CooldownTableTempPtr>;
    ...
    std::pair<const CooldownTableTempPtr, uint32_t> GetTable(uint32_t tableId);
    std::pair<const CooldownTableTempPtr, uint32_t> GetTableWithoutErrorLogging(uint32_t tableId);
