...
void MainSceneConfigurationTable::Load() {
    ...
    std::string path = GetConfigDir() + "mainscene.json";
...
std::pair<const MainSceneTable*, uint32_t> MainSceneConfigurationTable::GetTable(const uint32_t tableId) {
...
std::pair<const MainSceneTable*, uint32_t> MainSceneConfigurationTable::GetTableWithoutErrorLogging(const uint32_t tableId) {
    const auto it = kv_data_.find(tableId);
    if (it == kv_data_.end()) {
        return { nullptr, kInvalidTableId };
    }
