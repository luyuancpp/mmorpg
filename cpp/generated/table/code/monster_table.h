...
public:
    ...
    std::vector<const MonsterTable*> GetByIds(const std::vector<uint32_t>& ids) const {
    ...
    const MonsterTable* GetRandom() const {
    ...
    std::vector<const MonsterTable*> Filter(const std::function<bool(const MonsterTable&)>& pred) const {
        std::vector<const MonsterTable*> result;
    ...
    const MonsterTable* FindFirst(const std::function<bool(const MonsterTable&)>& pred) const {
        for (int i = 0; i < data_.data_size(); ++i) {
            if (pred(data_.data(i))) {
                return &data_.data(i);
            }
