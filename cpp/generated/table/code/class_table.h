...
public:
    ...
    struct Snapshot {
        ...
        std::unordered_multimap<uint32_t, const ClassTable*> idxskill;
    };
    ...
    const ClassTableData& FindAll() const { return snapshot_->data; }
    ...
    const std::unordered_multimap<uint32_t, const ClassTable*>& GetSkillIndex() const { return snapshot_->idxskill; }
    ...
    std::size_t CountBySkillIndex(uint32_t key) const { return snapshot_->idxskill.count(key); }
