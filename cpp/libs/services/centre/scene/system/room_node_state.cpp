...
void RoomNodeStateSystem::MakeNodePressure(entt::entity node) {
	tlsNodeContextManager.GetRegistry(eNodeType::RoomNodeService).emplace_or_replace<NodePressureComp>(node, NodePressureState::kPressure);
...
void RoomNodeStateSystem::ClearNodePressure(entt::entity node) {
	tlsNodeContextManager.GetRegistry(eNodeType::RoomNodeService).emplace_or_replace<NodePressureComp>(node, NodePressureState::kNoPressure);
...
void RoomNodeStateSystem::MakeNodeState(entt::entity node, NodeState state) {
	tlsNodeContextManager.GetRegistry(eNodeType::RoomNodeService).emplace_or_replace<NodeStateComp>(node, NodeState::kNormal);
