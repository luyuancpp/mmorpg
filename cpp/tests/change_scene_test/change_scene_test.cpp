...
entt::entity CreateMainSceneNodeEntity()
{
	...
	AddMainSceneToNodeComponent(tlsRegistryManager.actorRegistry, sceneEntity);
	return sceneEntity;
}
...
TEST(PlayerChangeScene, CreateMainScene)
{
	...
	for (uint32_t i = 0; i < 10; ++i)
	{
		...
		{
			globalSceneList.push_back(SceneCommon::CreateSceneOnSceneNode({ .node = mainSceneNode, .sceneInfo = sceneInfo }));
		}
	}
}
...
TEST(PlayerChangeScene, ChangeSameGsSceneNotEnqueue)
{
	...
	SceneCommon::EnterScene({ fromSceneEntity, playerEntity });
