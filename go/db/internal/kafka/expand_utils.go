...
// ExpandStatusData is the expansion status stored in Redis.
type ExpandStatusData struct {
	Status         string `json:"status"`
	PartitionCount int32  `json:"partition_count"`
	UpdateTime     int64  `json:"update_time"` // milliseconds
}

// SetExpandStatus writes expansion status to Redis.
...
// GetExpandStatus reads expansion status from Redis.
	...
	for {
		...
		case <-ticker.C:
			...
			for i := int32(0); i < oldPartitionCount; i++ {
				...
				//consumerGroup := config.AppConfig.ServerConfig.Kafka.GroupID
				currentOffset, err := client.GetOffset(topic, i, sarama.OffsetOldest)
				// Simplified: in production, use Redis/etcd to track consumer offsets
				if err != nil {
...
// GetNewPartitionIDs returns IDs for new partitions (e.g., old=3, new=5 → [3,4]).
