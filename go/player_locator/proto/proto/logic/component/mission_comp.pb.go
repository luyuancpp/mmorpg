// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.9
// 	protoc        v6.31.1
// source: proto/logic/component/mission_comp.proto

package component

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type MissionCompEMissionStatus int32

const (
	MissionComp_E_MISSION_NORMAL      MissionCompEMissionStatus = 0
	MissionComp_E_MISSION_ACHIEVEMENT MissionCompEMissionStatus = 1
	MissionComp_E_MISSION_COMPLETE    MissionCompEMissionStatus = 2
	MissionComp_E_MISSION_TIME_OUT    MissionCompEMissionStatus = 3
	MissionComp_E_MISSION_FAILD       MissionCompEMissionStatus = 4
)

// Enum value maps for MissionCompEMissionStatus.
var (
	MissionCompEMissionStatus_name = map[int32]string{
		0: "E_MISSION_NORMAL",
		1: "E_MISSION_ACHIEVEMENT",
		2: "E_MISSION_COMPLETE",
		3: "E_MISSION_TIME_OUT",
		4: "E_MISSION_FAILD",
	}
	MissionCompEMissionStatus_value = map[string]int32{
		"E_MISSION_NORMAL":      0,
		"E_MISSION_ACHIEVEMENT": 1,
		"E_MISSION_COMPLETE":    2,
		"E_MISSION_TIME_OUT":    3,
		"E_MISSION_FAILD":       4,
	}
)

func (x MissionCompEMissionStatus) Enum() *MissionCompEMissionStatus {
	p := new(MissionCompEMissionStatus)
	*p = x
	return p
}

func (x MissionCompEMissionStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MissionCompEMissionStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_logic_component_mission_comp_proto_enumTypes[0].Descriptor()
}

func (MissionCompEMissionStatus) Type() protoreflect.EnumType {
	return &file_proto_logic_component_mission_comp_proto_enumTypes[0]
}

func (x MissionCompEMissionStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MissionCompEMissionStatus.Descriptor instead.
func (MissionCompEMissionStatus) EnumDescriptor() ([]byte, []int) {
	return file_proto_logic_component_mission_comp_proto_rawDescGZIP(), []int{0, 0}
}

type MissionListCompEMissionType int32

const (
	MissionListComp_kPlayerMission       MissionListCompEMissionType = 0 //普通任务
	MissionListComp_kPlayerAchievment    MissionListCompEMissionType = 1 //成就
	MissionListComp_kPlayerDailyActivity MissionListCompEMissionType = 2 //每日活动
	MissionListComp_kPlayerMissionSize   MissionListCompEMissionType = 3 //最大值
)

// Enum value maps for MissionListCompEMissionType.
var (
	MissionListCompEMissionType_name = map[int32]string{
		0: "kPlayerMission",
		1: "kPlayerAchievment",
		2: "kPlayerDailyActivity",
		3: "kPlayerMissionSize",
	}
	MissionListCompEMissionType_value = map[string]int32{
		"kPlayerMission":       0,
		"kPlayerAchievment":    1,
		"kPlayerDailyActivity": 2,
		"kPlayerMissionSize":   3,
	}
)

func (x MissionListCompEMissionType) Enum() *MissionListCompEMissionType {
	p := new(MissionListCompEMissionType)
	*p = x
	return p
}

func (x MissionListCompEMissionType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (MissionListCompEMissionType) Descriptor() protoreflect.EnumDescriptor {
	return file_proto_logic_component_mission_comp_proto_enumTypes[1].Descriptor()
}

func (MissionListCompEMissionType) Type() protoreflect.EnumType {
	return &file_proto_logic_component_mission_comp_proto_enumTypes[1]
}

func (x MissionListCompEMissionType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use MissionListCompEMissionType.Descriptor instead.
func (MissionListCompEMissionType) EnumDescriptor() ([]byte, []int) {
	return file_proto_logic_component_mission_comp_proto_rawDescGZIP(), []int{1, 0}
}

type MissionComp struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            uint32                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	Status        uint32                 `protobuf:"varint,2,opt,name=status,proto3" json:"status,omitempty"`
	Progress      []uint32               `protobuf:"varint,3,rep,packed,name=progress,proto3" json:"progress,omitempty"`
	Data          []byte                 `protobuf:"bytes,4,opt,name=data,proto3" json:"data,omitempty"` // 各种类型buff的各种数据
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MissionComp) Reset() {
	*x = MissionComp{}
	mi := &file_proto_logic_component_mission_comp_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MissionComp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MissionComp) ProtoMessage() {}

func (x *MissionComp) ProtoReflect() protoreflect.Message {
	mi := &file_proto_logic_component_mission_comp_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MissionComp.ProtoReflect.Descriptor instead.
func (*MissionComp) Descriptor() ([]byte, []int) {
	return file_proto_logic_component_mission_comp_proto_rawDescGZIP(), []int{0}
}

func (x *MissionComp) GetId() uint32 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *MissionComp) GetStatus() uint32 {
	if x != nil {
		return x.Status
	}
	return 0
}

func (x *MissionComp) GetProgress() []uint32 {
	if x != nil {
		return x.Progress
	}
	return nil
}

func (x *MissionComp) GetData() []byte {
	if x != nil {
		return x.Data
	}
	return nil
}

type MissionListComp struct {
	state            protoimpl.MessageState         `protogen:"open.v1"`
	Missions         map[uint32]*MissionComp `protobuf:"bytes,1,rep,name=missions,proto3" json:"missions,omitempty" protobuf_key:"varint,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	CompleteMissions []byte                         `protobuf:"bytes,2,opt,name=complete_missions,json=completeMissions,proto3" json:"complete_missions,omitempty"`
	MissionBeginTime map[uint32]uint64              `protobuf:"bytes,3,rep,name=mission_begin_time,json=missionBeginTime,proto3" json:"mission_begin_time,omitempty" protobuf_key:"varint,1,opt,name=key" protobuf_val:"varint,2,opt,name=value"`
	Type             uint32                         `protobuf:"varint,4,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *MissionListComp) Reset() {
	*x = MissionListComp{}
	mi := &file_proto_logic_component_mission_comp_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *MissionListComp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MissionListComp) ProtoMessage() {}

func (x *MissionListComp) ProtoReflect() protoreflect.Message {
	mi := &file_proto_logic_component_mission_comp_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use MissionListComp.ProtoReflect.Descriptor instead.
func (*MissionListComp) Descriptor() ([]byte, []int) {
	return file_proto_logic_component_mission_comp_proto_rawDescGZIP(), []int{1}
}

func (x *MissionListComp) GetMissions() map[uint32]*MissionComp {
	if x != nil {
		return x.Missions
	}
	return nil
}

func (x *MissionListComp) GetCompleteMissions() []byte {
	if x != nil {
		return x.CompleteMissions
	}
	return nil
}

func (x *MissionListComp) GetMissionBeginTime() map[uint32]uint64 {
	if x != nil {
		return x.MissionBeginTime
	}
	return nil
}

func (x *MissionListComp) GetType() uint32 {
	if x != nil {
		return x.Type
	}
	return 0
}

type RewardListComp struct {
	state              protoimpl.MessageState `protogen:"open.v1"`
	CanRewardMissionId map[uint32]bool        `protobuf:"bytes,1,rep,name=can_reward_mission_id,json=canRewardMissionId,proto3" json:"can_reward_mission_id,omitempty" protobuf_key:"varint,1,opt,name=key" protobuf_val:"varint,2,opt,name=value"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *RewardListComp) Reset() {
	*x = RewardListComp{}
	mi := &file_proto_logic_component_mission_comp_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RewardListComp) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RewardListComp) ProtoMessage() {}

func (x *RewardListComp) ProtoReflect() protoreflect.Message {
	mi := &file_proto_logic_component_mission_comp_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RewardListComp.ProtoReflect.Descriptor instead.
func (*RewardListComp) Descriptor() ([]byte, []int) {
	return file_proto_logic_component_mission_comp_proto_rawDescGZIP(), []int{2}
}

func (x *RewardListComp) GetCanRewardMissionId() map[uint32]bool {
	if x != nil {
		return x.CanRewardMissionId
	}
	return nil
}

var File_proto_logic_component_mission_comp_proto protoreflect.FileDescriptor

const file_proto_logic_component_mission_comp_proto_rawDesc = "" +
	"\n" +
	"(proto/logic/component/mission_comp.proto\"\xf5\x01\n" +
	"\x12MissionComp\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\rR\x02id\x12\x16\n" +
	"\x06status\x18\x02 \x01(\rR\x06status\x12\x1a\n" +
	"\bprogress\x18\x03 \x03(\rR\bprogress\x12\x12\n" +
	"\x04data\x18\x04 \x01(\fR\x04data\"\x86\x01\n" +
	"\x0eeMissionStatus\x12\x14\n" +
	"\x10E_MISSION_NORMAL\x10\x00\x12\x19\n" +
	"\x15E_MISSION_ACHIEVEMENT\x10\x01\x12\x16\n" +
	"\x12E_MISSION_COMPLETE\x10\x02\x12\x16\n" +
	"\x12E_MISSION_TIME_OUT\x10\x03\x12\x13\n" +
	"\x0fE_MISSION_FAILD\x10\x04\"\xfd\x03\n" +
	"\x16MissionListComp\x12A\n" +
	"\bmissions\x18\x01 \x03(\v2%.MissionListComp.MissionsEntryR\bmissions\x12+\n" +
	"\x11complete_missions\x18\x02 \x01(\fR\x10completeMissions\x12[\n" +
	"\x12mission_begin_time\x18\x03 \x03(\v2-.MissionListComp.MissionBeginTimeEntryR\x10missionBeginTime\x12\x12\n" +
	"\x04type\x18\x04 \x01(\rR\x04type\x1aP\n" +
	"\rMissionsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\rR\x03key\x12)\n" +
	"\x05value\x18\x02 \x01(\v2\x13.MissionCompR\x05value:\x028\x01\x1aC\n" +
	"\x15MissionBeginTimeEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\rR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x04R\x05value:\x028\x01\"k\n" +
	"\feMissionType\x12\x12\n" +
	"\x0ekPlayerMission\x10\x00\x12\x15\n" +
	"\x11kPlayerAchievment\x10\x01\x12\x18\n" +
	"\x14kPlayerDailyActivity\x10\x02\x12\x16\n" +
	"\x12kPlayerMissionSize\x10\x03\"\xc1\x01\n" +
	"\x15RewardListComp\x12a\n" +
	"\x15can_reward_mission_id\x18\x01 \x03(\v2..RewardListComp.CanRewardMissionIdEntryR\x12canRewardMissionId\x1aE\n" +
	"\x17CanRewardMissionIdEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\rR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\bR\x05value:\x028\x01B6Z4player_locator/proto/proto/logic/component;componentb\x06proto3"

var (
	file_proto_logic_component_mission_comp_proto_rawDescOnce sync.Once
	file_proto_logic_component_mission_comp_proto_rawDescData []byte
)

func file_proto_logic_component_mission_comp_proto_rawDescGZIP() []byte {
	file_proto_logic_component_mission_comp_proto_rawDescOnce.Do(func() {
		file_proto_logic_component_mission_comp_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_proto_logic_component_mission_comp_proto_rawDesc), len(file_proto_logic_component_mission_comp_proto_rawDesc)))
	})
	return file_proto_logic_component_mission_comp_proto_rawDescData
}

var file_proto_logic_component_mission_comp_proto_enumTypes = make([]protoimpl.EnumInfo, 2)
var file_proto_logic_component_mission_comp_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_proto_logic_component_mission_comp_proto_goTypes = []any{
	(MissionCompEMissionStatus)(0),   // 0: MissionComp.eMissionStatus
	(MissionListCompEMissionType)(0), // 1: MissionListComp.eMissionType
	(*MissionComp)(nil),              // 2: MissionComp
	(*MissionListComp)(nil),          // 3: MissionListComp
	(*RewardListComp)(nil),           // 4: RewardListComp
	nil,                                     // 5: MissionListComp.MissionsEntry
	nil,                                     // 6: MissionListComp.MissionBeginTimeEntry
	nil,                                     // 7: RewardListComp.CanRewardMissionIdEntry
}
var file_proto_logic_component_mission_comp_proto_depIdxs = []int32{
	5, // 0: MissionListComp.missions:type_name -> MissionListComp.MissionsEntry
	6, // 1: MissionListComp.mission_begin_time:type_name -> MissionListComp.MissionBeginTimeEntry
	7, // 2: RewardListComp.can_reward_mission_id:type_name -> RewardListComp.CanRewardMissionIdEntry
	2, // 3: MissionListComp.MissionsEntry.value:type_name -> MissionComp
	4, // [4:4] is the sub-list for method output_type
	4, // [4:4] is the sub-list for method input_type
	4, // [4:4] is the sub-list for extension type_name
	4, // [4:4] is the sub-list for extension extendee
	0, // [0:4] is the sub-list for field type_name
}

func init() { file_proto_logic_component_mission_comp_proto_init() }
func file_proto_logic_component_mission_comp_proto_init() {
	if File_proto_logic_component_mission_comp_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_proto_logic_component_mission_comp_proto_rawDesc), len(file_proto_logic_component_mission_comp_proto_rawDesc)),
			NumEnums:      2,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_proto_logic_component_mission_comp_proto_goTypes,
		DependencyIndexes: file_proto_logic_component_mission_comp_proto_depIdxs,
		EnumInfos:         file_proto_logic_component_mission_comp_proto_enumTypes,
		MessageInfos:      file_proto_logic_component_mission_comp_proto_msgTypes,
	}.Build()
	File_proto_logic_component_mission_comp_proto = out.File
	file_proto_logic_component_mission_comp_proto_goTypes = nil
	file_proto_logic_component_mission_comp_proto_depIdxs = nil
}
