...
{
	SendToGsPlayer(messageId, message, tls_cl.get_player(playerId));
}

void SendToPlayerViaGs(uint32_t messageId, const google::protobuf::Message& message, Guid playerId)
{
	SendToPlayerViaGs(messageId, message, tls_cl.get_player(playerId));
}
...
{
	SendToPlayer(messageId, message, tls_cl.get_player(playerId));
}
