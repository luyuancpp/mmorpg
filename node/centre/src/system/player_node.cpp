...
void PlayerNodeSystem::HandlePlayerAsyncLoaded(Guid playerId, const player_centre_database& playerData)
{
	...
	if (const auto [first, success] = tlsCommonLogic.PlayerList().emplace(playerId, playerEntity); !success)
	{
...
void PlayerNodeSystem::HandlePlayerLeave(Guid playerUid)
{
	...
	defer(tlsCommonLogic.PlayerList().erase(playerUid));

	const auto playerEntity = tlsCommonLogic.get_player(playerUid);
