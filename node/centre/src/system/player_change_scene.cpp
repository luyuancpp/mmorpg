...
void PlayerChangeSceneSystem::CopySceneInfoToChangeInfo(CentreChangeSceneInfo& change_info, const SceneInfo& scene_info)
{
...
void PlayerChangeSceneSystem::TryProcessChangeSceneQueue(entt::entity player)
{
	...
	{
		//就算同gs,队列有消息也不能直接切换，
		ProcessSameGsChangeScene(player, change_scene_queue.front());
	}
	else if (change_scene_queue.front().change_gs_type() == CentreChangeSceneInfo::eDifferentGs)
	{
		ProcessDifferentGsChangeScene(player, change_scene_queue.front());
	}
}

void PlayerChangeSceneSystem::ProcessSameGsChangeScene(entt::entity player, const CentreChangeSceneInfo& change_info)
{
	const auto& dest_scene = entt::entity{ change_info.guid() };
...
void PlayerChangeSceneSystem::ProcessDifferentGsChangeScene(entt::entity player, const CentreChangeSceneInfo& change_info)
{
	//正在切换
	//切换gs  存储完毕之后才能进入下一个场景
	//放到存储完毕切换场景的队列里面，如果等够足够时间没有存储完毕，可能就是服务器崩溃了,注意，是可能
	if (change_info.change_gs_status() == CentreChangeSceneInfo::eLeaveGsScene)
	{
		ScenesSystem::LeaveScene({ player });
	}
	else if (change_info.change_gs_status() == CentreChangeSceneInfo::eEnterGsSceneSucceed)
	{
		//场景不存在了把消息删除,这个文件一定要注意这个队列各种异常情况
		const auto dest_scene = entt::entity{ change_info.guid() };
		if (entt::null == dest_scene)
