...
from core.paths import SRC_CPP, PROJECT_GENERATED_GO_PROTO_DIR, DATA_TABLES_DIR
...
def process_workbook(filepath: Path):
    ...
    try:
        ...
        for sheetname, data in workbook_data.items():
            ...
            PROJECT_GENERATED_GO_PROTO_DIR.mkdir(parents=True, exist_ok=True)
            generate_common.mywrite(go_content, PROJECT_GENERATED_GO_PROTO_DIR / f"{sheetname_lower}_table.go")
...
def generate_all_config():
    ...
    PROJECT_GENERATED_GO_PROTO_DIR.mkdir(parents=True, exist_ok=True)
    generate_common.mywrite(go_content, PROJECT_GENERATED_GO_PROTO_DIR / "all_table.go")
