...
func WriteServiceRegisterInfoFile() {
	...
	util.Wg.Add(1)
	go writePlayerServiceInstanceFiles("instance", isGsPlayerHandler, config.GsMethodHandleDir, config.GsPlayerMethodHandleDir)
	util.Wg.Add(1)
	go writePlayerServiceInstanceFiles("instance", isCentrePlayerHandler, config.CentreMethodHandleDir, config.CentrePlayerMethodHandleDir)
	util.Wg.Add(1)
