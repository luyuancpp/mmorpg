...
func WriteServiceRegisterInfoFile(wg *sync.WaitGroup) {
	...
	wg.Add(1)
	go writePlayerServiceInstanceFiles(wg, "instance", IsRoomNodeHostedPlayerProtocolHandler, _config.Global.PathLists.MethodHandlerDirectories.RoomNodePlayer, config.PlayerServiceName)
	wg.Add(1)
	go writePlayerServiceInstanceFiles(wg, "instance", IsCentreHostedPlayerServiceHandler, _config.Global.PathLists.MethodHandlerDirectories.CentreNodePlayer, config.PlayerServiceName)
	wg.Add(1)
	go writePlayerServiceInstanceFiles(wg, "repliedInstance", IsRoomNodeReceivedPlayerResponseHandler, _config.Global.PathLists.MethodHandlerDirectories.RoomNodePlayerReplied, config.PlayerRepliedServiceName)
	wg.Add(1)
	go writePlayerServiceInstanceFiles(wg, "repliedInstance", IsCentreReceivedPlayerServiceResponseHandler, _config.Global.PathLists.MethodHandlerDirectories.CentrePlayerReplied, config.PlayerRepliedServiceName)
	wg.Add(1)
	go writePlayerServiceInstanceFiles(wg, "instance", IsNoOpHandler, _config.Global.PathLists.MethodHandlerDirectories.GateNodePlayer, config.PlayerServiceName)
	wg.Add(1)
	go writePlayerServiceInstanceFiles(wg, "repliedInstance", IsNoOpHandler, _config.Global.PathLists.MethodHandlerDirectories.GateNodePlayerReplied, config.PlayerRepliedServiceName)
}
