...
func TestReadCodeSections_MethodBlocksNotMisclassifiedAsGlobal(t *testing.T) {
	...
	methods := buildParserTestMethods("CentrePlayerScene", "LeaveScene", "EnterScene")
	cpp := strings.Join([]string{
		"void CentrePlayerSceneHandler::LeaveScene(int a)",
		"{",
		...
		"}",
		"",
		"void CentrePlayerSceneHandler::EnterScene(int a)",
		"{",
		"///<<< BEGIN WRITING YOUR CODE",
		"\tenter_impl();",
		"///<<< END WRITING YOUR CODE",
	...
	leaveKey := "CentrePlayerSceneHandler::LeaveScene("
	enterKey := "CentrePlayerSceneHandler::EnterScene("
...
func buildParserTestMethods(serviceName string, methodNames ...string) RPCMethods {
	fd := &descriptorpb.FileDescriptorProto{Name: proto.String("proto/centre/player_scene.proto")}
