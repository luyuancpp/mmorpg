...
///<<< BEGIN WRITING YOUR CODE
	...
	const auto dest_scene = ScenesSystem::GetSceneByGuid(change_scene_info.scene_info().guid());
	const auto* const dest_game_node = tls.registry.try_get<GameNodePtr>(dest_scene);
