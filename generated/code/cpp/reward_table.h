...
public:
    ...
    std::vector<const RewardTable*> GetByIds(const std::vector<uint32_t>& ids) const {
    ...
    const RewardTable* GetRandom() const {
    ...
    std::vector<const RewardTable*> Filter(const std::function<bool(const RewardTable&)>& pred) const {
        std::vector<const RewardTable*> result;
    ...
    const RewardTable* FindFirst(const std::function<bool(const RewardTable&)>& pred) const {
        for (int i = 0; i < data_.data_size(); ++i) {
            if (pred(data_.data(i))) {
                return &data_.data(i);
            }
